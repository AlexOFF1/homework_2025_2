...
QUnit.module('Тестируем функцию groupBy', () => {
    ...
    QUnit.test('Бросает ошибку при невалидном первом аргументе (не массив)', (assert) => {
        assert.throws(
            () => groupBy('not an array', 'category'),
            Error,
            'Первый аргумент должен быть массивом',
            'Должна выбрасываться ошибка для невалидного массива'
        );
    });

    QUnit.test('Бросает ошибку при невалидном втором аргументе (не строка)', (assert) => {
        assert.throws(
            () => groupBy([], 123),
            Error,
            'Второй аргумент должен быть непустой строкой',
            'Должна выбрасываться ошибка для невалидного ключа'
        );
    });

    QUnit.test('Бросает ошибку при пустой строке ключа', (assert) => {
        assert.throws(
            () => groupBy([], ''),
            Error,
            'Второй аргумент должен быть непустой строкой',
            'Должна выбрасываться ошибка для пустого ключа'
        );
    });
